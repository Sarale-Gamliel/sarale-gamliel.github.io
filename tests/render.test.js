...
test('renderProjectCard shows a demo badge on the image corner when a demo link exists', () => {
  const html = renderProjectCard(liveProject, 'en', labels);
  assert.match(html, /project-card-demo-badge/);
...
test('renderProjectCard does not nest the demo badge anchor inside the image link', () => {
  const html = renderProjectCard(liveProject, 'en', labels);
  const imageLinkClose = html.indexOf('</a>');
  const demoBadgeOpen = html.indexOf('project-card-demo-badge');
  assert.ok(imageLinkClose > -1 && demoBadgeOpen > imageLinkClose, 'demo badge must be a sibling of the image link, not nested inside it');
});
